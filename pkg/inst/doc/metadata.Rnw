...
\begin{abstract}
	Package \Rpkg{marray} includes three S3 classes that provide 
	infrastructure for attaching metadata to data frames:
	\Rclass{marray}, \Rclass{dfMetadata} and \Rclass{varMetadata}.
...
\Rclass{marray} is an S3 class that extends \Rclass{data.frame}.     
\Rclass{marray}s behave mostly like data frames, with the following 
differences:
...
An \Rclass{marray} can be created in the same way as a data frame. The
...
mx <- marray(Num=1:3, Char=letters[1:3], Log=FALSE)
print(class(mx))
print(mx)
@ %def 
...
When an \Rclass{marray} is created without any additional parameters, the main 
